    ...
    class Network(AMPBuilder.Network):
        ...
        def form_embedding(self, task_out_z, obs_dict = None):
            ...
            if self.z_type == 'vae':  # here
                ...
                if self.use_vae_sphere_posterior: # False
        ...
        def compute_prior(self, obs_dict):
            ...
            if self.use_vae_prior: # True
                prior_logvar = self.z_prior_logvar(prior_latent)
                if self.use_vae_clamped_prior: # True
                    prior_logvar = torch.clamp(prior_logvar, min = -5, max = self.vae_var_clamp_max) # self.vae_var_clamp_max=2
        ...
        def eval_actor(self, obs_dict, return_extra = False):
            ...
            if self.has_rnn:
                ...
                if self.z_all: # False
                    ...
                    actor_input = torch.cat([self_obs, z_out], dim=-1)   # 778 + 48
        ...
        def dec_actor_from_z(self, z_out, obs_dict):
            obs = obs_dict['obs']
            # states = obs_dict.get('rnn_states', None)
            # seq_length = obs_dict.get('seq_length', 1)
