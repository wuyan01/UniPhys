...
class AMPPlayerContinuous(common_player.CommonPlayer):
    ...
    def _preproc_obs(self, obs_batch):
        ...
        if self.normalize_input:
            if obs_batch.shape[1] >= self.running_mean_std.mean_size:
                ...
                # import ipdb; ipdb.set_trace()
                obs_batch_proc = torch.zeros((obs_batch.shape[0], self.running_mean_std.mean_size)).to(obs_batch.device)
                obs_batch_proc[:, :obs_batch.shape[1]] = obs_batch
                obs_batch_out = self.running_mean_std(obs_batch_proc)
                obs_batch = obs_batch_out[:, :obs_batch.shape[1]]
