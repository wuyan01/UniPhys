...
import bisect
def find_labels(annotations, id, query_times):
    """
    Find text labels for given query time steps in batch-wise motion annotations.
    
    Args:
        annotations: List of lists, where each sublist contains dicts with 'start_t', 'end_t', and 'proc_label'.
        query_times: List of arrays, each containing query times for the corresponding motion.

    Returns:
        List of lists containing matched labels for each query time.
    """
    results = []
    
    # for motion_annotations, query_ts in zip(annotations, query_times):
    for i in id:
        motion_annotations = annotations[i]
        query_ts = query_times[i]
        # Sort annotations by start time for efficient search
        # motion_annotations = sorted(motion_annotations, key=lambda x: x['start_t'])
        
        # Extract start times for binary search
        # print(type(motion_annotations), motion_annotations)
        start_times = [seg['start_t'] for seg in motion_annotations]
        
        motion_results = []
        # Find the rightmost segment that starts before or at time t
        idx = bisect.bisect_right(start_times, query_ts) - 1
        
        # Check if the found segment actually contains t
        if 0 <= idx < len(motion_annotations) and motion_annotations[idx]['start_t'] <= query_ts <= motion_annotations[idx]['end_t']:
            motion_results.append(motion_annotations[idx]['proc_label'])
        else:
            motion_results.append("")  # No matching label found
...
class MotionLibBase():
    ...
    def load_motions(self, skeleton_trees, gender_betas, limb_weights, random_sample=True, start_idx=0, max_len=-1):
        ...
        if "frame_labels" in motion_file_data:
            self.frame_labels = np.array([m.frame_labels for m in motions], dtype=object)
            # self.frame_labels = np.array([sorted(m.frame_labels, key=lambda x: x['start_t']) for m in motions], dtype=object)
        else:
            self.frame_labels = np.array([None for m in motions])
    ...
    def get_frame_labels(self, motion_ids=None):
        if motion_ids is None:
            return self.frame_labels
        else:
            return self.frame_labels[motion_ids]
    ...
    def get_motion_state(self, motion_ids, motion_times, offset=None):
        ...
        # labels = find_labels(self.frame_labels, motion_ids, motion_times)
        labels = self.frame_labels[motion_ids]
