...
if __name__ == "__main__":
    ...
    # all_pkls = glob.glob("/cluster/project/tang/xialyu/LLM_Scene/Amass/amass_data/**/**/*.npz", recursive=True)
    all_pkls = glob.glob("/nfs/tang.scratch.inf.ethz.ch/export/tang/cluster/wuyan/data/amass_raw/**/**/*.npz", recursive=True)
    ...
    amass_splits = {
        ...
        'train_missing': ['BMLmovi', 'BMLhandball'] 
    }
    # 'ACCAD', 'BioMotionLab_NTroje'
    # import ipdb; ipdb.set_trace()
    process_set = amass_splits[process_split]
    length_acc = []
    for data_path in tqdm(all_pkls):
        bound = 0
        splits = data_path.split("/")[9:]
    ...
    # import ipdb; ipdb.set_trace()
    if upright_start:
        joblib.dump(amass_full_motion_dict, "data/amass/amass_{}_take6_upright.pkl".format(process_split), compress=True)
    else:
        joblib.dump(amass_full_motion_dict, "data/amass/amass_{}_take6.pkl".format(process_split), compress=True)
