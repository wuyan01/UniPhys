...
class Diffusion(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.action_dim = kwargs.get("action_dim", 0)
        self.state_dim = kwargs.get("state_dim", 0)
        ...
        if len(self.x_shape) == 3:
            ...
            if self.cfg.model_type == "enc":
                self.model = TransformerEncoder(
                    ...
                    k_emb_size=self.arch.k_emb_size,
                )

            elif self.cfg.model_type == "dec":
    ...
    def model_predictions(self, x, t, external_cond=None, force_mask=False):
        model_output = self.model(x, t, external_cond, force_mask, is_causal=self.is_causal)
        ...
        if self.objective == "pred_noise":
            ...
            x_start = model_output  ## x_0
            pred_noise = self.predict_noise_from_start(x, t, x_start) # x: x_t
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        if len(x_t) != len(t):
            assert len(t) == 2 * len(x_t)
            T = len(x_t)
            a_pred = (extract(self.sqrt_recip_alphas_cumprod, t[:T], x_t[..., :self.action_dim].shape) * x_t[..., :self.action_dim]
                      - extract(self.sqrt_recipm1_alphas_cumprod, t[:T], x_t[..., :self.action_dim].shape) * noise[..., :self.action_dim]
            )
            s_pred = (extract(self.sqrt_recip_alphas_cumprod, t[T:], x_t[..., self.action_dim:].shape) * x_t[..., self.action_dim:]
                      - extract(self.sqrt_recipm1_alphas_cumprod, t[T:], x_t[..., self.action_dim:].shape) * noise[..., self.action_dim:]
            )
            return torch.cat([a_pred, s_pred], dim=-1)
        else:
            return (
    ...
    def predict_noise_from_start(self, x_t, t, x0):
        if len(x_t) != len(t):
            assert len(t) == 2 * len(x_t)
            T = len(x_t)
            a_pred = (extract(self.sqrt_recip_alphas_cumprod, t[:T], x_t[..., :self.action_dim].shape) * x_t[..., :self.action_dim] - x0[..., :self.action_dim]) / extract(
                self.sqrt_recipm1_alphas_cumprod, t[:T], x_t[..., :self.action_dim].shape
            )
            s_pred = (extract(self.sqrt_recip_alphas_cumprod, t[T:], x_t[..., self.action_dim:].shape) * x_t[..., self.action_dim:] - x0[..., self.action_dim:]) / extract(
                self.sqrt_recipm1_alphas_cumprod, t[T:], x_t[..., self.action_dim:].shape
            )
            return torch.cat([a_pred, s_pred], dim=-1)
        else:
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        if len(x_start) != len(t):
            T = len(x_start)
            assert len(t) == 2*T
            a = (extract(self.sqrt_alphas_cumprod, t[:T], x_start[..., :self.action_dim].shape) * x_start[..., :self.action_dim]
                + extract(self.sqrt_one_minus_alphas_cumprod, t[:T], x_start[..., :self.action_dim].shape) * noise[..., :self.action_dim])
            s = (extract(self.sqrt_alphas_cumprod, t[T:], x_start[..., self.action_dim:].shape) * x_start[..., self.action_dim:]
                + extract(self.sqrt_one_minus_alphas_cumprod, t[T:], x_start[..., self.action_dim:].shape) * noise[..., self.action_dim:])
    ...
    def p_mean_variance(self, x, t, external_cond=None, force_mask=False):
        model_pred = self.model_predictions(x=x, t=t, external_cond=external_cond, force_mask=force_mask)
    ...
    def forward(
        ...
        model_pred = self.model_predictions(x=noised_x, t=noise_levels, external_cond=external_cond, force_mask=force_mask)
        ...
        if len(loss_weight) != len(loss):
            T = len(loss)
            a_loss_weight = loss_weight[:T]
            s_loss_weight = loss_weight[T:]
            loss_weight = torch.zeros_like(loss).cuda()
            loss_weight[..., :self.action_dim] = a_loss_weight.unsqueeze(-1)
            loss_weight[..., self.action_dim:] = s_loss_weight.unsqueeze(-1)
        else:
    ...
    def ddim_sample_step(
        ...
        **kwargs
        ...
        # clipped_curr_noise_level = curr_noise_level
        # treating as stabilization would require us to scale with sqrt of alpha_cum
        orig_x = x.clone().detach()
        scaled_context = self.q_sample(
            x,
        ...
        if guidance_fn is not None:
            with torch.enable_grad():
                ...
                guidance_loss = guidance_fn(model_pred.pred_x_start, **kwargs)
